...
function defaultStore() {
  return {
    server: {
      ...
      tagline: 'Tienda oficial de PoseidonHCF'
    },
    theme: {
      primary: '#297bff',
      secondary: '#54e5ff',
    ...
    content: {
      ...
      heroSubtitle: 'Rangos, cosméticos, prefixes, killtags y servicios automáticos en una sola tienda.',
    ...
    links: [],
    discounts: [
      { id: 'global-20', name: 'Oferta general', percent: 20, active: true, description: '20% de descuento en toda la tienda.', scope: 'global', categoryId: '' }
    ],
    categories: [
    ...
    products: [
      ...
      { id: 'prefix-dev', categoryId: 'prefix', name: 'Prefix en desarrollo', price: 0.89, currency: 'USD', badge: 'Dev', description: 'Sección reservada para prefix futuros.', features: ['Placeholder', 'En desarrollo', 'Editable luego'], image: { from: '#8892ff', to: '#35e8ff', accent: 'rgba(136,146,255,.4)' } },
      { id: 'cosmetic-dev', categoryId: 'cosmetics', name: 'Cosméticos en desarrollo', price: 0.89, currency: 'USD', badge: 'Dev', description: 'Bloque para cosméticos más adelante.', features: ['Placeholder', 'Editable', 'Visual limpio'], image: { from: '#7c5cff', to: '#35e8ff', accent: 'rgba(124,92,255,.4)' } },
      
      { id: 'supporter', categoryId: 'donations', name: 'Donación Supporter', price: 0.90, currency: 'USD', badge: 'Donate', description: 'Aporte simple para apoyar el proyecto.', features: ['Apoyo al server', 'Sin obligación', 'Ayuda real'], image: { from: '#f4c96b', to: '#35e8ff', accent: 'rgba(244,201,107,.35)' } },
      { id: 'boost', categoryId: 'donations', name: 'Donación Boost', price: 2.70, currency: 'USD', badge: 'Donate', description: 'Aporte medio para ayudar al crecimiento.', features: ['Ayuda extra', 'Apoyo directo', 'Muy útil'], image: { from: '#ff8ab3', to: '#7c5cff', accent: 'rgba(255,138,179,.35)' } },
      { id: 'ppkg', categoryId: 'more', name: 'PPKG EnderChest', price: 2.19, currency: 'USD', badge: 'Extra', description: 'EnderChest que da abilitys.', features: ['EnderChest', 'Abilitys', 'Buen precio'], image: { from: '#35e8ff', to: '#f4c96b', accent: 'rgba(53,232,255,.38)' } },
      { id: 'unmute', categoryId: 'unban', name: 'Unmute', price: 3.49, currency: 'USD', badge: 'Unban', description: 'Desmuteo para restaurar tu chat y volver a hablar en el servidor.', features: ['Desmuteo rápido', 'Nick vinculado', 'Entrega automática'], image: { src: '/assets/unban/unbanlogo.png', from: '#34c7ff', to: '#7b6dff', accent: 'rgba(52,199,255,.42)' } },
      { id: 'unban', categoryId: 'unban', name: 'Unban', price: 6.99, currency: 'USD', badge: 'Unban', description: 'Desbaneo estándar para volver a entrar con la cuenta sancionada.', features: ['Desbaneo estándar', 'Válido para Minecraft', 'Proceso simple'], image: { src: '/assets/unban/unbanlogo.png', from: '#2d7bff', to: '#35e8ff', accent: 'rgba(45,123,255,.42)' } },
      { id: 'unstrike', categoryId: 'unban', name: 'Unstrike', price: 5.49, currency: 'USD', badge: 'Unban', description: 'Quita una sanción tipo strike y deja el historial listo para seguir jugando.', features: ['Retiro de strike', 'Historial limpio', 'Atención prioritaria'], image: { src: '/assets/unban/unbanlogo.png', from: '#f4c96b', to: '#ff8ab3', accent: 'rgba(244,201,107,.36)' } },
      { id: 'unban-cheats', categoryId: 'unban', name: 'Unban para Cheats', price: 11.99, currency: 'USD', badge: 'Unban', description: 'Opción especial para sanciones relacionadas con cheats.', features: ['Revisión manual', 'Sanciones graves', 'Mayor prioridad'], image: { src: '/assets/unban/unbanlogo.png', from: '#ff8ab3', to: '#7c5cff', accent: 'rgba(255,138,179,.38)' } }
    ]
  };
}
...
function getDiscountPercentForProduct(store, product) {
  const discounts = Array.isArray(store?.discounts) ? store.discounts : [];
  let percent = 0;
  for (const discount of discounts) {
    if (discount?.active === false) continue;
    const value = Math.max(0, Math.min(100, Number(discount?.percent || 0) || 0));
    if (!value) continue;
    const scope = String(discount?.scope || 'global').toLowerCase();
    if (scope === 'global') {
      percent = Math.max(percent, value);
    } else if (scope === 'category' && product && String(discount?.categoryId || '').trim() && String(discount.categoryId).trim() === String(product.categoryId || '').trim()) {
      percent = Math.max(percent, value);
    }
  }
  return percent;
...
function applyDiscountPercent(amount, percent) {
  ...
  const pct = Math.max(0, Math.min(100, Number(percent || 0) || 0));
  return Number((value * (1 - (pct / 100))).toFixed(2));
}

function getDiscountedAmount(store, product, amount) {
  return applyDiscountPercent(amount, getDiscountPercentForProduct(store, product));
}
...
function resolveOrderAmount(store, product, customAmount) {
  ...
  if (product?.allowCustomAmount) {
    ...
    resolved = Number(amount.toFixed(2));
  }

  return getDiscountedAmount(store, product, resolved);
}
...
app.post('/api/create-order', async (req, res) => {
  try {
    ...
    const amount = resolveOrderAmount(store, product, customAmount);
  ...
  try {
    ...
    res.json({
      ...
      reward: {
        ...
        productName: product.name,
        successMessage: rewardCode.successMessage || `¡Felicidades! Código canjeado a la cuenta ${String(minecraftNick).trim()}.`
      }
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
