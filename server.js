...
app.post('/api/create-order', async (req, res) => {
  try {
    const { minecraftNick, productId } = req.body || {};
    ...
    const order = await paypalCreateOrder(Number(product.price), product.currency || 'USD', `${product.name} • ${minecraftNick}`);
...
app.post('/api/redeem-gift-card', (req, res) => {
  try {
    const { minecraftNick, productId, code } = req.body || {};
    if (!minecraftNick || String(minecraftNick).trim().length < 3) {
      return res.status(400).json({ error: 'Minecraft nick is required' });
    }
