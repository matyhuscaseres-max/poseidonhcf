...
const refs = {
  ...
  customAmountPanel: document.getElementById('custom-amount-panel'),
  ...
  rewardCodeInput: document.getElementById('reward-code-input'),
  rewardCodeRedeemBtn: document.getElementById('reward-code-redeem-btn'),
  ...
  addRewardCodeBtn: document.getElementById('add-reward-code-btn'),
  rewardCodeEditor: document.getElementById('reward-code-editor'),
...
const FALLBACK_ICONS = {
  ranks: '👑', prefix: '🏷️', killtags: '☠️', cosmetics: '✨', keys: '🗝️', donations: '💙', more: '⚡', unban: '⛔'
};
...
function getProductDiscountPercent(product) {
  const discounts = Array.isArray(state.store?.discounts) ? state.store.discounts : [];
  let percent = 0;
  for (const discount of discounts) {
    if (discount?.active === false) continue;
    const value = normalizePercent(discount?.percent);
    if (!value) continue;
    const scope = String(discount?.scope || 'global').toLowerCase();
    if (scope === 'global') {
      percent = Math.max(percent, value);
    } else if (scope === 'category' && product && String(discount?.categoryId || '').trim() === String(product.categoryId || '').trim()) {
      percent = Math.max(percent, value);
    }
  }
  return percent;
...
function applyDiscountPercent(amount, percent) {
  const value = Number(amount || 0);
  const pct = normalizePercent(percent);
  return Number((value * (1 - pct / 100)).toFixed(2));
}

function getDisplayPrice(product, amount = Number(product?.price || 0)) {
  return applyDiscountPercent(amount, getProductDiscountPercent(product));
}
...
function showRewardOverlay({ product, minecraftNick, code, successMessage }) {
  if (!refs.rewardOverlay) return;
  const imageSrc = product?.image?.src || '';
  const displayNick = String(minecraftNick || '').trim();
  if (refs.rewardImage) {
    refs.rewardImage.src = imageSrc || '/assets/logos/logo.png';
    refs.rewardImage.alt = product?.name || 'Premio';
  }
  if (refs.rewardMessage) {
    refs.rewardMessage.textContent = successMessage || `¡Felicidades! Código canjeado a la cuenta ${displayNick}.`;
  }
  if (refs.rewardMeta) {
    refs.rewardMeta.textContent = `${product?.name || 'Premio'} · ${displayNick}${code ? ` · Código ${code}` : ''}`;
  }
  refs.rewardOverlay.classList.remove('hidden');
  refs.rewardOverlay.setAttribute('aria-hidden', 'false');
}

function hideRewardOverlay() {
  if (!refs.rewardOverlay) return;
  refs.rewardOverlay.classList.add('hidden');
  refs.rewardOverlay.setAttribute('aria-hidden', 'true');
}
  ...
  try {
    ...
    const result = await api('/api/redeem-gift-card', {
      ...
      body: JSON.stringify({
        ...
        code,
        customAmount: product.allowCustomAmount ? getSelectedProductAmount(product) : undefined
      })
    });
...
async function redeemRewardCode() {
  try {
    const minecraftNick = refs.minecraftNick.value.trim();
    const code = refs.rewardCodeInput.value.trim();
    if (minecraftNick.length < 3) {
      refs.checkoutFeedback.textContent = 'Ingresa un usuario válido antes de canjear.';
      return;
    }
    if (!code) {
      refs.checkoutFeedback.textContent = 'Escribe el código de regalo.';
      return;
    }

    const result = await api('/api/redeem-reward-code', {
      method: 'POST',
      body: JSON.stringify({ minecraftNick, code })
    });
    refs.rewardCodeInput.value = '';
    const product = result.product || state.store?.products.find((item) => item.id === (result.order?.productId || ''));
    showRewardOverlay({
      product,
      minecraftNick,
      code: result.reward?.code || code,
      successMessage: `¡Felicidades, ${minecraftNick}! Código canjeado a la cuenta ${minecraftNick}.`
    });
    refs.checkoutFeedback.textContent = `${result.reward.successMessage} Premio: ${result.reward.productName}.`;
    await bootstrap();
  } catch (e) {
    refs.checkoutFeedback.textContent = e.message || 'No se pudo canjear el código.';
  }
}
...
function getSelectedProductAmount(product = getSelectedProduct()) {
  if (!product) return 0;
  const discountPercent = getProductDiscountPercent(product);
  if (!product.allowCustomAmount) {
    return applyDiscountPercent(Number(product.price || 0), discountPercent);
  }

  const fallback = Math.max(0, Number(product.minPrice ?? product.price ?? 0));
  const amount = Number(refs.customAmountInput?.value || fallback);
  const baseAmount = Number.isFinite(amount) && amount > 0 ? amount : fallback;
  return applyDiscountPercent(baseAmount, discountPercent);
}

function syncCustomAmountUI(product = getSelectedProduct()) {
  const enabled = Boolean(product?.allowCustomAmount);
  refs.customAmountPanel?.classList.toggle('hidden', !enabled);
  if (!enabled || !refs.customAmountInput) return;

  const minPrice = Math.max(0, Number(product.minPrice ?? product.price ?? 0));
  refs.customAmountInput.min = String(minPrice || 0);
  refs.customAmountInput.placeholder = `Mínimo ${minPrice.toFixed(2)}`;
  const current = Number(refs.customAmountInput.value || 0);
  if (!Number.isFinite(current) || current < minPrice) {
    refs.customAmountInput.value = minPrice.toFixed(2);
  }
}
...
function openProductInfo(productId) {
  ...
  if (refs.productInfoPrice) {
    const discounted = getDisplayPrice(product);
    const discount = getProductDiscountPercent(product);
    refs.productInfoPrice.innerHTML = discount > 0
      ? `<span class=\"product-price-original\">${formatPrice(product.price, product.currency)}</span> ${formatPrice(discounted, product.currency)}`
...
function renderCatalog() {
  ...
  products.forEach((product) => {
    ...
    let saleBadge = card.querySelector('.product-sale-badge');
    if (!saleBadge) {
      saleBadge = document.createElement('span');
      saleBadge.className = 'product-sale-badge';
      card.querySelector('.product-cover')?.appendChild(saleBadge);
    }
    ...
    price.innerHTML = discountPercent > 0
      ? `<span class=\"product-price-original\">${formatPrice(product.price, product.currency)}</span> ${formatPrice(discountedPrice, product.currency)}`
      : formatPrice(product.price, product.currency);
...
function populateProductSelect() {
  ...
  (state.store?.products || []).forEach((p) => {
    ...
    opt.value = p.id;
    const discounted = getDisplayPrice(p);
    const discountPercent = getProductDiscountPercent(p);
    opt.textContent = discountPercent > 0 ? `${p.name} - ${formatPrice(discounted, p.currency)}` : `${p.name} - ${formatPrice(p.price, p.currency)}`;
...
function updateCheckoutSummary() {
  ...
  syncCustomAmountUI(product);
  const amount = getSelectedProductAmount(product);
  refs.checkoutName.textContent = product.name;
  refs.checkoutPrice.textContent = formatPrice(amount, product.currency);
    ...
    async createOrder() {
      ...
      const data = await api('/api/create-order', {
        ...
        body: JSON.stringify({
          ...
          customAmount: selected.allowCustomAmount ? getSelectedProductAmount(selected) : undefined
        })
      });
...
function renderAdminStats(stats) {
  ...
  [
    ...
    { label: 'Canjes de código', value: stats.rewardCodeRedemptions || 0 },
...
function createProductCard(product = {}) {
  ...
  card.innerHTML = `
    ...
    <div class="product-edit-grid">
      ...
      <label class="toggle-row"><input class="product-allow-custom-amount" type="checkbox" ${p.allowCustomAmount ? 'checked' : ''} /> <span>Monto libre</span></label>
      <input class="input product-min-price" type="number" step="0.01" min="0" placeholder="Monto mínimo" value="${Number(p.minPrice || 0)}" />
...
function createRewardCodeCard(rewardCode = {}) {
  const defaults = {
    id: `reward-code-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
    code: generateGiftCode().replace('GIFT', 'REWARD'),
    productId: state.store?.products?.[0]?.id || '',
    maxUses: 1,
    active: true,
    note: '',
    successMessage: '',
  ...
  const item = { ...defaults, ...rewardCode };
  const card = document.createElement('article');
  card.className = 'product-edit-card reward-code-edit-card';
  card.dataset.createdAt = item.createdAt;
  card.innerHTML = `
    <div class="product-edit-head">
      <strong>${safeText(item.code)}</strong>
      <button class="button ghost remove-btn" type="button">Eliminar</button>
    </div>
    <div class="product-edit-grid">
      <input class="input reward-code-value" placeholder="Código" value="${safeText(item.code).replace(/"/g, '&quot;')}" />
      <select class="input reward-code-product"></select>
      <input class="input reward-code-max-uses" type="number" min="1" step="1" placeholder="Usos máximos" value="${Number(item.maxUses || 1)}" />
      <input class="input reward-code-expires-at" type="datetime-local" value="${safeText(item.expiresAt || '').replace(/"/g, '&quot;')}" />
      <label class="toggle-row"><input class="reward-code-active" type="checkbox" ${item.active !== false ? 'checked' : ''} /> <span>Activo</span></label>
      <input class="input reward-code-note" placeholder="Nota interna" value="${safeText(item.note || '').replace(/"/g, '&quot;')}" />
      <textarea class="textarea reward-code-success-message" placeholder="Mensaje al canjear">${safeText(item.successMessage || '')}</textarea>
      <p class="helper">Canjeado: ${Array.isArray(item.claims) ? item.claims.length : 0} / ${Number(item.maxUses || 1)}</p>
    </div>
  `;

  const select = card.querySelector('.reward-code-product');
  (state.store?.products || []).forEach((product) => {
    const opt = document.createElement('option');
    opt.value = product.id;
    opt.textContent = product.name;
    if (product.id === item.productId) opt.selected = true;
    select.appendChild(opt);
  });

  card.querySelector('.remove-btn').addEventListener('click', () => card.remove());
  card.querySelector('.reward-code-value').addEventListener('input', (e) => {
    card.querySelector('.product-edit-head strong').textContent = e.target.value || 'Reward code';
  });
  return card;
}

function readRewardCodesFromEditor() {
  return [...refs.rewardCodeEditor.querySelectorAll('.reward-code-edit-card')].map((card) => {
    const code = card.querySelector('.reward-code-value').value.trim().toUpperCase().replace(/\s+/g, '');
    const previous = state.store?.rewardCodes?.find((item) => item.code === code);
    return {
      id: code ? code.toLowerCase().replace(/[^a-z0-9]+/g, '-') : `reward-code-${Date.now()}`,
      code,
      productId: card.querySelector('.reward-code-product').value,
      maxUses: Math.max(1, Number(card.querySelector('.reward-code-max-uses').value || 1)),
      active: Boolean(card.querySelector('.reward-code-active').checked),
      note: card.querySelector('.reward-code-note').value.trim(),
      successMessage: card.querySelector('.reward-code-success-message').value.trim(),
      expiresAt: card.querySelector('.reward-code-expires-at').value.trim(),
      claims: previous?.claims || [],
      createdAt: card.dataset.createdAt || new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };
  }).filter((item) => item.code && item.productId);
}
...
function fillAdminForm(store, orders) {
  ...
  if (refs.rewardCodeEditor) {
    refs.rewardCodeEditor.innerHTML = '';
    (store.rewardCodes || []).forEach((rewardCode) => refs.rewardCodeEditor.appendChild(createRewardCodeCard(rewardCode)));
  }

  refs.salesTableBody.innerHTML = '';
  orders.forEach((o) => {
    const row = document.createElement('tr');
    const paymentLabel = o.paymentMethod === 'gift_card'
      ? 'Tarjeta de regalo'
      : (o.paymentMethod === 'reward_code' ? 'Código regalo' : 'PayPal');
...
function readProductsFromEditor() {
  return [...refs.productEditor.querySelectorAll('.product-edit-card')].map((card) => {
    ...
    const allowCustomAmount = Boolean(card.querySelector('.product-allow-custom-amount').checked);
    const minPrice = Number(card.querySelector('.product-min-price').value || 0);
    const image = {
...
async function saveStore() {
  try {
    const payload = {
      ...
      rewardCodes: readRewardCodesFromEditor()
    };
...
refs.customAmountInput?.addEventListener('input', () => updateCheckoutSummary());
refs.enterStoreBtn?.addEventListener('click', enterStore);
refs.giftCardPayBtn?.addEventListener('click', redeemGiftCard);
refs.rewardCodeRedeemBtn?.addEventListener('click', redeemRewardCode);
refs.playerName?.addEventListener('keydown', (e) => { if (e.key === 'Enter') enterStore(); });
refs.rewardClose?.addEventListener('click', hideRewardOverlay);
refs.rewardOk?.addEventListener('click', hideRewardOverlay);
refs.rewardOverlay?.addEventListener('click', (e) => { if (e.target?.dataset?.closeReward !== undefined) hideRewardOverlay(); });
document.addEventListener('keydown', (e) => { if (e.key === 'Escape') hideRewardOverlay(); });
...
refs.addRewardCodeBtn?.addEventListener('click', () => refs.rewardCodeEditor.appendChild(createRewardCodeCard()));
